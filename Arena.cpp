...
: m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nZombies(0), m_history(nRows, nCols)   //initializing m_history?
{
...
History& Arena::history()       //making the history function that returns a reference to a history object?
{
    return m_history;
}
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    ...
    if (k < m_nZombies  &&  m_zombies[k]->getAttacked(dir))  // zombie dies
    {
        ...
        m_nZombies--;
        m_history.record(m_player->row(),m_player->col());      //send to record players col and row at time of attack
        return true;
    }
    return false;
}
